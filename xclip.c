...
// TODO: stop uncommenting here for writing
//struct requestor
//{
//        Window cwin;
//        Atom pty;
...
// TODO: start uncommenting here for writing
//static int
//doIn(Display *dpy, Window win, Atom sseln, Atom target,
//     const char *progname, unsigned char *sel_buf, unsigned long sel_len)
//{
...
static int
doOut(Display *dpy, Window win, Atom sseln, Atom target,
      unsigned char **sel_buf_out, unsigned long *sel_len_out)
{
    Atom sel_type = None;
    unsigned char *sel_buf;     /* buffer for selection data */
    unsigned long sel_len = 0;  /* length of sel_buf */
    ...
    while (1) {
        ...
        int ret = xcout(dpy, win, evt, sseln, target, &sel_type, &sel_buf, &sel_len, &context);
        if (ret < 0) {
            return -1;
        }
...
int get_dpy(char *sdisp, Display **dpy_out, Window *win_out) {
    Display *dpy;
    Window win;
...
void close_dpy(Display *dpy) {
    /* Disconnect from the X server */
    XCloseDisplay(dpy);
}

int get_clipboard(Display *dpy, Window win, unsigned char **sel_buf_out,
                  unsigned long *sel_len_out) {
    return doOut(dpy, win, XA_CLIPBOARD(dpy), XA_UTF8_STRING(dpy), sel_buf_out,
                 sel_len_out);
}

int set_clipboard(Display *dpy, Window win, unsigned char **sel_buf_out,
                  unsigned long *sel_len_out) {
    return doOut(dpy, win, XA_CLIPBOARD(dpy), XA_UTF8_STRING(dpy), sel_buf_out,
                 sel_len_out);
}
